...
- ✅ **Förarbete correctness tail (2026-07-19):** lr/SÖ bodies recovered by
  ...
  data-table model with cross-page continuation (now `lib/tabell.py`); the
...
accommodanda/
  ...
  lib/      shared horizontal libs (full map: accommodanda/README.md "Shared library (lib/)") — lagrum (citation engine), catalog, page (the shared page kit) + render (site assembly) + tpl and the Jinja templates/ their markup lives in, layout, net, markdown, util, errors, casenaming, eucasenaming, labels, eu_structure, datasets, search, facets, feeds, dump, pins, resolve, text, compress, facsimile, pdftext, tabell, llm, annstore, wikitext, runlog, patch·patchit, markup, git, harvest, regeringen, poi, concepts, diff, history, assets, coe, coe_ids, pinpoint
  ...
  forarbete/ preparatory-works vertical — download·propkb·kbtitles·soukb·riksdagen·rskr·model·parse·volumes·structure·kommentar·genomforande·aigenomforande·fk·jamforelse·lydelse·legacy_formats
  ...
  lawreview/ journal-article vertical, nine journals (svjt·jp·ft·nmt·njel·siplr·urt·euar·lod) — journals·download·<journal>·lawpub·model·parse (nine journals plus the lawpub.se platform scope; mined for citations only: no pages, feed or index of their own; a rail line links to the journal's own page; svjt and lod walk on harvest watermarks)
...
- ✅ **Shared PDF parser** (`foreskrift/{parse,structure}.py`) — one parser for all 15 fs.
  ...
  - **Five printed page shapes the prose reflow used to flatten** (2026-08-24), each
    measured over the corpus before the fix and after. The **ingress** — the preamble
    stating the beslutsdatum and the bemyndigande, required by 18 b §
    författningssamlingsförordningen (1976:725) — was dropped with the masthead by
    11 538 of 11 899 regulations; `_ingress_start` walks back from the body to the last
    rubrik, masthead furniture line or title semicolon. The **page-foot footnotes**
    (`lib/pdftext.ruled_footnotes`) reach an artifact `footnotes` key; the rule of
    underscores alone is not evidence, since 3 920 regulations draw the same rule above
    their ikraftträdande clause, so the lines below it must also be set smaller than the
    page's running size. The **ordförklaringar table** is reconstructed from the page
    geometry (`lib/tabell.split_two_column`, joined across page breaks). The **bullet
    lists** glued into one stycke (7 688 blocks) become `lista`/`punkt`. The **allmänna
    råd** (6 419 blocks across 895 regulations) become their own `allmanna_rad` sections —
    advisory text must not read as a binding stycke, and a råd is cut at the document's
    closing matter so it cannot swallow the ikraftträdande clause and the signature.
    Rubrik depth is ranked over the body's own heading sizes, so a chaptered document's
    table of contents nests instead of listing chapter and subheading side by side.
    **The whole corpus needs a reparse to gain these.**
...
| `accommodanda/lib/tabell.py` | **table detection from the `pdftext` Line/run geometry**, shared by the PDF-bodied sources (moved out of `forarbete/` when föreskrift became its second caller). `split_generic` reads a *data table* on numeric evidence (förarbete budget tables, bilaga listings); `split_two_column` reads a *two-column prose table* — a föreskrift's ordförklaringar, a term beside its sentence-long definition — which numeric evidence would reject, keying on the layout instead and taking the vertical step as the row boundary so a wrapped term keeps its definition. `merge_continued` joins either across a page break |
...
- **lawpub** (2026-08-24) — lawreview's tenth scope,
  ...
  two separate basefiles (`lawreview/ft/...` and `lawreview/lawpub/...`).
  `test/test_lawpub.py`.
