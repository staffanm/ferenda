...
- Implemented verticals: **SFS** (statutes), **DV** (court decisions),
  ...
  **rs** (agency legal positions), **edpb** (EU data-protection guidance),
...
1. **Vertical source pipelines** (`accommodanda/{sfs,dv,hudoc,coe,icrc,untc,icc,eurlex,edpb,forarbete,foreskrift,avg,remisser,wiki}/`)
...
- `scope` is a vertical (`sfs`, `dv`, `hudoc`, `coe`, `icrc`, `untc`, `icc`,
  `eurlex`, `edpb`, `forarbete`, `foreskrift`, `avg`, `rs`, `remisser`, `wiki`) or a
