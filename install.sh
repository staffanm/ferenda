...
echo "downloading version from $GCS_BUCKET/stable"

# Always download stable version (which has the most up-to-date installer)
version=$(download_file "$GCS_BUCKET/stable")

echo "version $version, downloading manifest from $GCS_BUCKET/$version/manifest.json"
...
echo "expected checksum is $checksumn now downloading $GCS_BUCKET/$version/$platform/claude to $binary_path" 
