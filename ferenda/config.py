"""Runtime configuration for the ferenda pipeline.
...
# follow. This origin serves every namespace that ``lib.layout`` can mint.
...
def resolve_editors(doc):
    ...
    by ``python -m ferenda.api.auth hash``; no plaintext password is ever
    ...
    for user, entry in raw.items():
        ...
        if not _RE_PWHASH.match(entry["pwhash"]):
            raise ConfigError("editor %r has a malformed pwhash at %s -- mint one "
                              "with `python -m ferenda.api.auth hash`"
                              % (user, loc))
