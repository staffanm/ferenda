"""Login and session handling for the inline content editor, plus
`same_origin`, the gate that the whole internal surface carries.
...
``hashlib.pbkdf2_hmac``); ``python -m ferenda.api.auth hash`` mints one to
...
def _pwhash_fingerprint(pwhash):
    ...
    revocation mechanism: sessions carry no server-side state (a cookie
    survives a restart) and the site has a
