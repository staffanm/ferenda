...
TPL = tpl.environment("ferenda.api").get_template("ops.html").module
...
def _version():
    """The running ferenda revision: the git sha baked into the image at
    build (FERENDA_GIT_SHA, .git being dockerignored), else -- in a dev
    working tree -- the live short HEAD, else 'unknown'."""
    return (os.environ.get("FERENDA_GIT_SHA")
