...
def case_uri(cid):
    ...
    produces: `dom/{serie}/{year}:{nr}`, `dom/nja/{year}s{page}`, or
    `.../not/{n}`. This published identifier must not change.
    ...
    date needed to call :func:`verdict_uri`; only a case missing one of those
    facts stays on the slug."""
