...
the edges from either source. That walk, and the `documents` row an artifact
becomes, live in `catalog_rows` -- everything relate derives from one artifact
alone, with no connection in sight. This module is the part that needs the
database: the schema, the writes, and every query the site and the API read
the catalog through.
"""
...
def uri_local(uri: str) -> str:
    """The local id of the document a (possibly pinpointed) uri names:
    `local(strip_fragment(uri))`, the pair every uri-keyed API route makes."""
...
def quiesce_wal(path: Path | str) -> None:
    ...
    (`corpus._swap_catalog`): SQLite pairs a `-wal` with a database by *filename*,
...
def artifact_for(con, stored):
    """The parsed artifact JSON behind a documents row of `con` -- the pairing
    of `data_root` and `load_artifact` every API read surface makes."""
    return load_artifact(data_root(con), stored)
...
def _index_document(con, art, path, source):
    ...
    display = (lb.official_title if source == "eurlex" and art.get("doctype") == "treaty"
               else catalog_rows.display_title(art, row[4]))
    ...
    con.execute(
        ...
        (*row, art.get("source_url"),
         ...
         catalog_rows._document_description(art, source),
         catalog_rows._document_snippet(art, source),
    ...
    edges = catalog_rows.artifact_links(art) + [
        (anchor, None, run)
        for anchor, run in (catalog_rows.subject_links(art) + definition_links(art)
...
def canonicalize_concepts(con):
    ...
    canonical node (`lib.begrepp`): cluster every referenced concept + wiki
...
def dangling_anchors(con, sources):
    ...
    (`corpus.ANCHOR_EXACT`), which is also what keeps the pass cheap: only those
...
def document(con: sqlite3.Connection, uri: str) -> dict | None:
    ...
    descriptive, source_url), or None -- the metadata behind an API /document
    lookup.

    `descriptive` and `source_url` sit last so the positional reads of the
    first six (path is `row[5]` in three callers) keep their places."""
    return con.execute(
...
def card_row(con: sqlite3.Connection, uri: str):
    ...
    COALESCEd with `description` so a catalog stamped before the snippet column
    still answers for every court decision. None for a uri the catalog does not
    hold."""
    return con.execute(
...
def link_count(con: sqlite3.Connection, from_uri: str, to_root: str) -> int:
    """How many citations `from_uri` makes to the document `to_root` -- the
    weight of one hop of a citation chain (api/app._hop_count)."""
    return con.execute(
        "SELECT count(*) FROM links WHERE from_uri = ? AND to_root = ?",
        (from_uri, to_root)).fetchone()[0]
...
def documents(con, source=None, kind=None, limit=None, offset=0,
    ...
    search). `display` is the reader-facing heading (catalog_rows.display_title).
