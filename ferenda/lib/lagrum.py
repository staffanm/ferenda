...
Differences from the reference scanner:
...
// item-link -- the reference scanner read "§ 26 första stycket" that way
...
def _fragment_context(basefile, fragment):
    """Decompose a minted fragment id into baseuri attributes. Bilaga fragments
    yield law-only context because the grammar stops at a B segment. A bare
    ...
    against the full basefile, for example "1736:0123_1#K9P2". The reference
    corpus collapses both balkar to "1736:0123#…"; the golden adjudication
...
def _law_id_span(law_node):
    ...
    law word), not the surrounding "lagen ( … )" scaffolding."""
...
class LagrumParser:
    ...
    def link_spans(self, attrlist, tree, length):
        ...
        into the nearest preceding link they contiguously follow. Links without an emitted
    ...
    def fmt_external_ref(self, node, match, out, context):
        ...
        # combines too. The reference scanner split it (format_ExternalRef's
