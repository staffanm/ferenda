...
Fragment ids replicate the reference projection's URI minting exactly, including
...
def inline_references(nodes, frag=""):
    ...
    tuples the reference projection emitted, from the links now inlined into the
    ...
    the reference projection never scanned them (the new one does)."""
    refs = []
    for node in nodes:
        kind = node["type"]
...
def iter_overgangar(doc):
    ...
    Overgangsbestammelser containers, which the reference projection allowed at top
...
class Projection:
    ...
    def inline(self, text, context, live=True, subject_terms=()):
        ...
        citation graph anyway -- the reference projection omitted it too.
