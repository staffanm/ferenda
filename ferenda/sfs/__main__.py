"""CLI for the new SFS pipeline.
  ...
  python -m ferenda.sfs parse DOWNLOADED.html         # NF JSON to stdout
  python -m ferenda.sfs validate GOLDENDIR DOWNLOADDIR  # corpus-wide compare
"""
...
def describe(problem):
    ...
    golden being the *expected* value and Ferenda the *actual* one.
    ...
    if m:
        kind, uri = m.groups()
        return (("amendment only in golden (Ferenda omitted it)", uri, "—")
                if kind == "missing" else
...
def cmd_refs(args):
    ...
    counterpart (the reference-projection parsed XHTML, normalized on the fly, or a
...
def cmd_validate(args):
    # The golden input is parsed XHTML+RDFa from the sibling reference checkout.
    # validate_one normalizes each file to NF on demand.
    ...
    if accepted_rules:
        ...
        for rule, n in accepted_rules.most_common():
            print("  %5d  %s" % (n, rule))
    print("top regression buckets (golden = expected, Ferenda = actual):")
...
def main():
    ...
    v.add_argument("parseddir", help="reference-projection parsed XHTML tree "
    ...
    v.add_argument("--report", default="/tmp/ferenda-validate.json")
