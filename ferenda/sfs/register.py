...
# label -> URI for orgs (foaf:name) and series (skos:altLabel), extracted from
# the curated Swedish legal source vocabulary: a FOAF.name pass followed by a
# SKOS.altLabel pass, with the first subject per label winning.
...
def build_metadata(sfst_header, register, basefile):
    ...
    # the consolidated SFST header is authoritative (the reference projection let it
