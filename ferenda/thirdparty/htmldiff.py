...
def htmldiff(old_html, new_html, include_hrefs=True, tagfilters=[]):
    ...
    *fragments* (str/UTF8 or unicode), they are not complete documents
    ...
    """ 
    old_html_tokens = tokenize(old_html, include_hrefs, tagfilters)
    new_html_tokens = tokenize(new_html, include_hrefs, tagfilters)
...
def tokenize(html, include_hrefs=True, tagfilters=[]):
    """
    ...
    for tagfilter in tagfilters:
        for el in body_el.xpath(tagfilter):
            skipped_elements.append(el)
    chunks = list(flatten_el(body_el, skip_tag=True, include_hrefs=include_hrefs, skipped_elements=skipped_elements))
...
def flatten_el(el, include_hrefs, skip_tag=False, skipped_elements=[]):
    ...
    for child in el:
        for item in flatten_el(child, include_hrefs=include_hrefs, skipped_elements=skipped_elements):
            yield item
