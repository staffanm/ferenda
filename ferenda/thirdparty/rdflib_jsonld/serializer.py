...
class Converter(object):
    ...
    def add_to_node(self, graph, s, p, o, s_node, nodemap):
        ...
        if not term:
            if language:
                term = context.find_term(str(p), None, LANG, language)
            elif not is_literal:
                term = context.find_term(str(p), ID) or context.find_term(str(p), VOCAB)
