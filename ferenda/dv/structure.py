...
The body (Rubrik/Stycke in document order) is grouped into the decision tree
that the DV structural golden expects:
...
classify a block by editorial cues, and a marker of
...
The parser needs no per-court state machine or recognizer-priority tables.
It uses the markers, nesting ranks, and one rule that the precise court name
(from the appellant-action wording, e.g. "väckte
