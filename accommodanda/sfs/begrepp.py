...
säkerhetsskydd (säkerhetsskyddsanalys)."). Each becomes a ``dcterms:subject``
inline link over the term's own span (``Ref`` with ``kind="term"``)."""
...
def paragraf_mode(stycke_texts, in_appendix=False):
    ...
    övergångsbestämmelser, where a mid-sentence parenthesis does not announce a
    definition: an annex is a list of things, not drafting. See `defined_terms`
    for what it costs to ignore."""
    ...
    # triggers above, so it used to yield no terms at all. Added strictly last
    # and only when nothing else matched: letting it override an announced mode
    # turned term-list paragrafs into parentes ones and lost their colon-list
    # terms.
    if (mode is None and not in_appendix
            and any(_has_coinage(t) for t in stycke_texts)):
        mode = "parantes"
    return mode


def _stycke_terms(text, mode, in_appendix=False):
    """Every term this stycke defines: the sentence-final coinages first, then
    the mid-sentence ones -- not document order, which matters because
    `nf.inline` gives each span to the terms already taken.

    Precedence is the old sequential overwrite: a parenthesised coinage beats a
    löptext definition, which beats a brottsrubricering, which beats a colon
    list. Only the parenthesis case can yield more than one term."""
    term = None
    ...
    for m in re_parantesdefs(text):
        ...
        if not found:
            continue
        # a parenthesis closing the sentence is a coinage whatever its shape --
        # that is the long-standing rule, and it outranks a colon-list term the
        # same way it always did. One away from the end has to look like a term.
...
def defined_terms(text, mode, kind, in_appendix=False):
    """The terms defined by this node (possibly empty), sentence-final coinages
    before mid-sentence ones.
    `kind` is 'stycke', 'listelement' or 'tabellrad'; for a table row `text` is
    the first cell. Only a stycke can define more than one term -- a list item
    and a table row each name exactly one.

    `in_appendix` turns the mid-sentence coinage rule off. An annex is a list of
    things rather than drafting, and the same brackets there hold table cells
    and the English half of a dubbelbeskattningsavtal: of 1,238 mid-sentence
    candidates over 1,500 acts, the 198 inside a bilaga, a table or
    övergångsbestämmelser held nearly all of the worst noise ("ton", "stat nr",
    "réseau", "alone or together with the whole enterprise"). A parenthesis that
    closes a sentence still counts there, exactly as it always has."""
    if kind == "tabellrad":
        ...
        terms = _stycke_terms(text, mode, in_appendix)
    kept = []
    for term in terms:
