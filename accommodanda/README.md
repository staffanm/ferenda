...
| `summaries.py` | `lagen hudoc sync-summaries` — the Court's own Case-Law Information Notes (6,505 in English) as a **link on the case they summarise**, never a second document saying the same thing. HUDOC gives a note no pointer to its case, so the join is `(application number, date)`, which is unique across the store (`held_index` raises on a collision). **Both this and `propose-translations` need a single-language store**: every language expression of a case repeats its application numbers, its date and its ECLI, so a store harvested with `--lang ENG,FRE` trips both joins on the first bilingual case, deliberately — choosing a language for the link to hang on is a rule nobody has written. One sidecar per case under `<downloaded>/hudoc/clin/<itemid>.json`, so a re-run re-stales only the parses whose summary moved; a summary the Court withdraws has its sidecar reaped, and an empty match set raises rather than reaping all of them |
| `translations.py` | `lagen hudoc propose-translations` — Domstolsverkets 87 Swedish translations (2014-01-14 … 2015-12-15) are commentary on the judgment they translate, not documents of their own: this drafts `commentary/hudoc/<itemid>.md` in the content repo, the inverse of the English-translation link `commentary/sfs/1971/291.md` opens with. Joined on the ECLI, which a translation shares exactly with its original; a translation whose original is not held is reported, never guessed |
...
uv run python -m accommodanda.build hudoc download               # English judgments + decisions, incremental
...
uv run python -m accommodanda.build hudoc sync-summaries         # link the Court's own summaries to their cases
uv run python -m accommodanda.build hudoc propose-translations   # draft commentary for the 87 Swedish translations
