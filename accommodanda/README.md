...
| `labels.py` | one place for every source's four reader-facing name forms — `short_id` (eyebrow), `short_title` (h1), `official_title` (dl.meta "Titel"), `descriptive_label` (compact citing form, I1) — dispatched per source (`_sfs`/`_eurlex`/`_dv`/`_forarbete`/`_foreskrift`/`_avg`/`_hudoc`/`_coe`/`_icrc`/`_untc`/`_icc`, else `_generic`) over the artifact dict's own parse-time stamps plus the curated datasets (`NAMEDLAWS`, `COE_NAMES`, `ICRC_NAMES`, `UNTC_TREATIES`, the new `eurlex/data/treaties.json` for EU primary law); read by both `render.py` (every per-document page) and `catalog.py` (the stamped `descriptive` column) so the two can't drift. `sfs_is_statute` also lives here (moved from `facets.py`): the title-based lag/balk/grundlag test that stamps every SFS row's catalog `kind` (`lag` vs `forordning`, no longer one uniform `law`) and, through it, its rung in the norm hierarchy (`catalog.norm_level`) |
...
| `eu_structure.py` | the one EU-act sub-article anchor grammar, entered through `Anchors` (the running article/paragraph/point context; `anchored_blocks`/`flatten` walk it over a document, `subarticle_key`/`citable` are its internals), shared by the eurlex parser, the renderer and the wiki guidance layer (`nest`, the parse-time tree builder, stays in `eurlex/structure.py`) |
| `regeringen.py` | shared regeringen.se harvest knowledge — the doctype table (`TYPES`: url segment, taxonomy category id, identifier regex), the `ul.list--block` listing walk (`listing_items`), and the identity rules for the two doctypes regeringen.se publishes without a series number (`pm_identity`, moved out of `forarbete/download.py`; `lr_identity`) — both must mint the same basefile for the same document whichever page names it, so forarbete and remisser share the rule rather than each guessing; used by `forarbete/download.py` and `remisser/download.py` |
...
| `templates/` | the render layer's markup: `page.html` (site chrome: masthead, frontmatter, grid, mobile toolbar — every page's base template), (each source's own page template now lives in `<source>/templates/`, extending page.html with the page body), `nodes.html` (document-tree leaf macros the node walks emit through), `partials/` (`meta`/`banners`/`panels`/`rail` macro libraries), `listings.html` (frontpage, folkrätt, browse, feed directory), `sok.html`/`admin.html`, `dataset_feed.html` (the chrome-free live feed twin). Template edits re-stale generate (`build.GENERATE_CODE`); output equivalence is checked browser-level by `tools/render_equivalence.py` (HTML5-normalized snapshots + compare) |
| `assets/` | the browser-facing static chrome as real on-disk files (`style.css`, `editor.css`, `dom.js`, `scrollspy.js`, `search.js`, `popover.js`, `fullsearch.js`, `versions.js`, `faksimil.js`, `drawers.js`, `editor.js`, `robots.txt`) — formerly embedded string constants in `render.py`. `render.write_assets` ships them through the same Brotli precompression as pages: the JS files are concatenated in load order into a single **`script.js`** bundle (the page links one URL, so adding a module changes only the bundle, never the per-page HTML — it publishes via `generate --assets-only`, not a full regenerate), `style.css` is written as `fonts/fonts.css` + `style.css` + `editor.css`, `robots.txt` copied as-is. `fonts/` holds the **self-hosted** Inter + Source Serif 4 woff2 files (variable fonts, one per family/style × latin/latin-ext with a `font-weight` range covering the weights the pages use, served under `/fonts/` and stored plain — woff2 is already compressed); the pages load **no third-party resource at all**, and the only client state is the sanctioned theme localStorage plus the editor's `lagen_editor` session cookie. `dom.js` (first in the bundle) is the scripts' shared vocabulary (`window.lagenDom`: own-document anchor resolution across split-view panes, id-attribute selector, landing flash, JSON-island parse); `search.js` leads the ⌘K palette with instant *local* hits (a terse pinpoint — `4`, `11:2`, `art 5`, `(42`, `skäl 42`, `bilaga III` — resolved against the current page's own anchors, no network); `popover.js` gives every internal reference a hover preview of the target's rendered text (replacing the old title-attribute tooltip) whose ↗ escalates to a split reading view — the target document in its own pane with its own TOC/rail and scrollspy instance (`lagenScrollspy`), resizable/reorderable/closable; `versions.js` carries the "jämför med tidigare lydelse" diff select *and* re-decides, against the reader's own clock, which of a consolidated statute's sibling variants is out of force — the build stamps `temporal-expired`/`temporal-pending` from its own date, which a nightly rebuild keeps within a day, and the `data-upphor`/`data-ikraft` attributes let a page cached past midnight still dim the right half |
| `feeds.py` | legacy dataset-alias map (`sfs`/`dv`/`forarbeten`/`myndfs`/`myndprax`/`keyword`/`eurlex` → the rebuilt source names) + the feed renderers — Atom stays Python string-building (XML with a byte-stable contract), the HTML twin renders `templates/dataset_feed.html` — shared by static `/dataset/<alias>/feed[.atom]` generation and the live query-param endpoints in `api/app.py` |
...
| `tabell.py` | conservative generic data-table detection for everything tabular that isn't a lydelse comparison (budget tables, bilaga listings, multi-column enumerations) — `split_generic` requires ≥3 consecutive multi-cell lines agreeing on ≥2 column starts before believing a region is a table (TOC dot-leader lines and page-margin markers excluded), `merge_continued` joins a table split across a page break, dropping the repeated header; wired into `parse.py`'s block assembly alongside `lydelse.py`'s comparison tables |
...
**edpb vertical (Europeiska dataskyddsstyrelsens riktlinjer och rekommendationer)**
| File | What |
|---|---|
| `series.py` | the data registry: the two open series (**riktlinjer**, **rekommendationer**) with their citation form, plus the **closed** third — the artikel 29-gruppens vägledningar the EDPB endorsed whole on 25 May 2018 (Endorsement 1/2018 names sixteen, all carried), enumerated individually with the Commission newsroom item id that actually holds each one's text — except the two BCR application forms, which the working party issued as Word *forms*, so no authoritative PDF of them exists and each entry names the tillsynsmyndighets conversion it takes instead, with what that conversion was verified against (WP 264 against a second authority's independent conversion, WP 265 against the working party's own Word file). The newsroom compounds it for WP 264: its item's download link serves the WP263 PDF. They are listed rather than harvested because the EDPB publishes a document page for only eight of them, and the seven of those under `/documents/guideline/` are stubs: five carry no file at all, one links an unrelated Danish decision, WP250's page is titled "Dataskyddsombud" (WP243's subject), and two separate pages exist for each of WP242 and WP260. One endorsed document has no WP number and no cover, so its title and adoption date are written down here too. `number_slug` normalises the löpnummer's padding, since the EDPB writes "05/2020" and "1/2018" in the same series and a citation copies whichever it saw |
| `model.py` | `Vagledning` model; URI = `edpb/{serie}/{nummer}` (`edpb/riktlinjer/05-2020`, `edpb/wp/248`) — the avg/rs grammar with the series in place of the myndighet, and the issuer's own number as the identity. These documents have **no CELEX**, which is what keeps them out of eurlex. Two fields are first-class rather than decoration: `sprak` (52 of 60 exist in Swedish, the rest in English only, and a page showing English text must say so) and `version` (a riktlinje is adopted, consulted on and re-adopted, so serving version 1 as current would misstate what the board says — and would be exactly the distortion the EDPB's reuse terms forbid). The numbered **punkt** is the citable unit and becomes the anchor (`#punkt27`), which is what a decision citing "punkt 27 i riktlinjer 05/2020" needs |
| `download.py` | two routes. **The EDPB site** (Drupal, no API — `/jsonapi` 404s) is indexed off its five-page `sitemap.xml`, with the document type in the URL path; each document is read twice, once per language, because the two pages carry different things — the *English* page always states the number (the Swedish one sometimes drops it), the *Swedish* page carries the title and links the Swedish PDF. `series_number` falls back to the six-digit token in the EDPB's own file name where the title states no number at all (the newest guidelines have stopped), telling `ÅÅÅÅNN` from `NNÅÅÅÅ` by which half is a year the EDPB has existed in. **The Commission newsroom** holds the WP29 documents; their Swedish translations sit inside 10–28 MB per-language ZIPs, so `swedish_member` extracts `wp<N>…_sv.pdf` (never the `…_annex_sv.pdf` of the second ZIP, and `_SE.pdf` too — one archive names Swedish by the country code) and only the extracted PDF is stored. Not every item carries such a ZIP, and one carries a 7-Zip archive instead, so those documents are published in English. That corpus is closed, so a routine run does not re-resolve it |
| `parse.py` | the body layer is `lib/pdftext.classify_letterhead` under `by_size` (the EDPB sets no bold anywhere: body 17, sections 24, title 27 — the same reading Finansinspektionens ställningstaganden need). What is EDPB-specific is the **numbered punkt**: the number is set in a column of its own, which the paragraph-gap heuristic cannot see — paragraph 17 of Riktlinjer 05/2020 arrived glued to the end of 16 — so `numbered_breaks` reads the numbers as a *running sequence* (a line opening "N. " counts only when N is the one the document is due next, so a year or a list item cannot start a paragraph) and hands them to `page_paragraphs` as forced breaks, the mechanism DV's bitmap paragraph numbers use. `join_continuations` rejoins a paragraph split by a page break — but only where the numbering actually covers the document (`PUNKT_COVERAGE_MIN`), since a working document that numbers its *sections* "1." and "2." otherwise has every paragraph under a section glued onto its heading, which made WP 250 a single 46,000-character block. `wp_cover` reads the WP29 title and adoption date off the document's own Swedish cover — the one field taken from the document rather than the index, because the index is demonstrably wrong about it — or, for the one endorsed document that sets no cover, from the registry. `titled` corrects the four Swedish pages where the EDPB left the English heading standing. `footnotes` picks up what the block classifier drops (`lib/pdftext.letterhead_footnotes`) — a riktlinje's apparatus lives in its notes, 1,236 of them carrying 2,020 citations. Two parsers, Swedish and English, since three documents are English-only and the same act citation reads differently in each |
...
| `lang.py` | localized structural vocabulary for the non-Formex (html/pdf) parsers (Formex is tag-marked, so its parser needs no language knowledge) and for `annotate.py`'s annex trim, which reads the same `VOCAB[lang]["annex"]` words back out of already-parsed text |
...
| `render.py` | the EU-rättsaktssida: articles, recitals and the editorial annotation layer. Registered as this source's page renderer in `build.SOURCE_RENDERERS`; `render(art, site) -> str`, built on the `lib/page` kit, with its own page template in `templates/` |
...
| `render.py` | the FN-fördragssida: the instrument and its parties. Registered as this source's page renderer in `build.SOURCE_RENDERERS`; `render(art, site) -> str`, built on the `lib/page` kit, with its own page template in `templates/` |
...
exception handlers on the same app (`errors.install(app)`) — a rendered
`error.html` page for site paths, JSON with an added `error_id` for
`/api/`/`/docs`/`/redoc`/`/openapi.json`/`/mcp`, and one `lib/errorlog.py`
...
```sh
uv run python -m accommodanda.build avg download        # all five organs; or: … download jo
...
uv run python -m accommodanda.build avg download imy    # tillsyner + the two curated pages
uv run python -m accommodanda.build avg download kkv    # tillsynsbeslut + ärendelista
```
...
**rs — rättsliga ställningstaganden from six myndigheter** (operates on
`site/data/{downloaded,artifact}/rs/`):

```sh
uv run python -m accommodanda.build rs download          # all six; or: … download fk
uv run python -m accommodanda.build rs parse             # incremental, like every source
uv run python -m accommodanda.build rs download fk --only fk/2025:01   # one statement
uv run python -m accommodanda.build rs download migr     # Lifos (RS + RK), AIA-completed TLS
```
...
```sh
uv run python -m accommodanda.build remisser download                    # harvest new ärenden + re-poll open ones
uv run python -m accommodanda.build remisser download --only <arende-url>  # one ärende, bypassing the listing walk
uv run python -m accommodanda.build remisser parse                       # incremental, like every source
uv run python -m accommodanda.build remisser ai-analyze <typ>/<document id>/<org-slug>  # the sole LLM pass, one answer
uv run python -m accommodanda.build remisser ai-analyze <typ>/<document id>              # whole ärende: every answer still lacking a layer
```
...
IN `site/data/`: their bodies were copied into the ordinary `downloaded/`
tree and their records rewritten to the harvested `files` form, same as any
live-harvested document (§7g teardown, 2026-07-19) — there is no longer a
separate legacy-corpus mount to configure.
...
uv run python -m accommodanda.build all errors [N]     # newest N served-site errors (default 50), newest first
uv run python -m accommodanda.build all errors <id>    # one error in full, traceback included (the 8-hex id its error page showed)
