...
| `datasets.py` | canonical filesystem paths of the curated named-resource datasets (`NAMEDLAWS`/`NAMEDACTS`/`NAMEDCASES`/`NAMEDEUCASES`/`COE_NAMES`/`ICRC_NAMES`/`UNTC_TREATIES`/`ICC_DECISION_TYPES`/`TREATY_NAMES`/`FS_SERIES`/`EMD_CASES`/`EMD_RESPONDENTS`) that ship in the package tree; `load_emd_cases` is the pure-JSON reader for the committed hudoc case-name snapshot |
...
| File | What |
|---|---|
| `model.py` | `Measure` (the on-disk `kind` discriminator the renderer dispatches on: `scalar`/`toplist`/`series`/`histogram`/`bars`/`profile`/`matrix`/`table`; a `profile` draws every value at its rank, largest first, so each bar is a real thing's own size rather than a bucket count) + `Row`/`Point`/`Cell`, and `Report.to_artifact()` which prunes a measure's empty fields so two builds' artifacts diff readably. A population caveat has two homes, neither a footnote: a computed `lede` states the exclusions with their counts (measure 4 names all five), and the page template's own `note=` renders under the figure for the caveat that is prose rather than arithmetic (the EU measures 4 and 6 carry one) |
...
citation graph, version history + diff, `GET /api/v1/pdf` — a generated page
...
images — all resolved in-process, never over the network) that also serves the static site under
...
tree (`lib/datasets.py`'s `NAMEDLAWS`/`NAMEDACTS`/`NAMEDCASES`/`NAMEDEUCASES`/`COE_NAMES`/`ICRC_NAMES`/`UNTC_TREATIES`/`ICC_DECISION_TYPES`/`TREATY_NAMES`/`EMD_CASES`/`EMD_RESPONDENTS`,
...
```sh
uv run python -m accommodanda.build rs download          # the six HTTP agencies; or: … download fk
...
uv run python -m accommodanda.build rs browser-download  # Skatteverket, weekly, headful Chrome
```
...
uv run python -m accommodanda.build hudoc download               # judgments + decisions, then the Court's own
                                                                 # summaries and the Swedish translations
uv run python -m accommodanda.build hudoc download decisions     # one collection (each has its own watermark)
uv run python -m accommodanda.build hudoc download --limit 1000  # bounded: the two smaller harvests are skipped
uv run python -m accommodanda.build hudoc parse
uv run python -m accommodanda.build hudoc casenames               # rewrite hudoc/data/casenames.json from the stored records
...
uv run python -m accommodanda.build untc download                # the 14 curated treaties: status page + authentic text
uv run python -m accommodanda.build untc parse                   # both halves -> metadata, participation and articles
...
site/data/artifact/stats/statistik.json       # the 52 corpus measurements (no downloaded/ half — the corpus is the input)
