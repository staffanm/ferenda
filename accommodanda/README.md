...
| `render.py` | the EU-rättsaktssida: articles, recitals and the editorial annotation layer. Registered as this source's page renderer in `build.SOURCE_RENDERERS`; `render(art, site) -> str`, built on the `lib/page` kit, with its own page template in `templates/`. An article heading hangs its designation in a gutter -- the word small over the number, a pilcrow under it -- beside the article's own title; `_article_parts` undoes the join the parser makes (TI.ART + " - " + STI.ART, the first run the article's self-reference). A division heading is all capitals in the sources ("KAPITEL I ALLMÄNNA BESTÄMMELSER"): `_division_label` splits the designation off and `_case_map`/`_sentence_case` re-set the title in sentence case, taking each word's capitals from the act's own prose rather than from a list of proper nouns -- so "kommissionen" lowercases while "Europeiska centralbanken" and "96/9/EG" keep theirs. A word needs *consistent* evidence (twice, and more than twice as often as the act writes it lowercase); a single cross-reference is not evidence. Only an all-caps heading that carries a designation is touched -- a free-form one ("FÖRLAGA TILL INTYG ... AMERIKAS FÖRENTA STATER") is left exactly as published, since its names appear nowhere else in the act to read a case from |
...
authenticated, mutating surface (see "Inline editing" below); `api/graphicsedit.py`
...
`api/facsimiles.py` is the shared responder that turns one `lib/facsimile.py`
render into a PNG response — `parse_bbox`, `png_response` (which owns the
poppler exit-code split: a missing page is a 404, a corrupt PDF stays a loud
500) and the immutable cache headers. It is its own module rather than
`app.py`'s privates because `app` imports every router, so a router importing
`app` back would be a cycle; `app.py`, `api/graphics.py` and the legacy
facsimile paths all read it. `api/analytics.py`
...
`GET /api/v1/graphics/review` (see "Reviewing `.graphics` crops" below).
...
public render until a human signs it off. `GET /api/v1/graphics/review`
