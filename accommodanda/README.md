...
| File | What |
|---|---|
| `agencies.py` | the data registry driving one shared harvest engine — 71 registered författningssamlingar (the full lagrummet.se agency list, county `\d+FS` series excluded), 66 live + 5 frozen-only (rsfs, sosfs/hslffs, sjvfs, svkfs, §7g), no per-agency pipelines; SKVFS and MTFS alone select detached headful Chrome, all others use requests/HTTP2 |
