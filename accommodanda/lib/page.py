...
``render(art, site) -> str`` registered in ``build.SOURCE_RENDERERS``); this
module owns what they all stand on:
  ...
  * the context rail -- :class:`Rail` / :class:`RailSection` and the margin
    builders, which turn a paragraph's inbound citations into the JSON island
    the client swaps as you scroll. That round-trip (case -> paragraph -> back
    to every case on that paragraph) is the signature lagen.nu feature.
