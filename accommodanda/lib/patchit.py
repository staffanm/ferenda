# -*- coding: utf-8 -*-
"""Vendored unified-diff parser + fuzzy applier.

A near-verbatim copy of the old pipeline's ``ferenda.thirdparty.patchit`` (itself
a fork of https://github.com/staffanm/patchit), pulled in here because the old
``ferenda`` package no longer imports under modern Python. Self-contained --
stdlib only (``bisect``/``re``/``collections``) -- so it drops in cleanly. One
documented fix vs. the original: a hunk header may omit a length of 1 (see
``PatchSetReader.__handle_hunk``).

Unlike ``patch(1)`` this applies a diff *in memory* to a list of lines and, via
``Patch.adjust``, tolerates the source having drifted by a few lines from what
the patch was cut against (it re-anchors on the hunk's context lines).
``lib.patch`` wraps it; no other module should import it directly.
"""
import bisect
import re
        ...
        for symbol, text in self.operations:
            if symbol in (Hunk.OP_EQUAL, Hunk.OP_DELETE, ):
                try:
                    ...
                    raise PatchConflictError('Unexpected end of stream') from None
...
class PatchSetReader(object):
    ...
    def __handle_hunk(self, hunk_dict):
        ...
        # a unified-diff hunk header may omit the length when it is 1
        # (`@@ -1 +1 @@`, as difflib emits for a single-line range) -- the
        # spec reads a missing count as 1. (Local fix to the vendored source;
        # the original assumed the count was always present and int(None)'d.)
        a_range = (int(hunk_dict['a1']), int(hunk_dict['a2'] or 1), )
        b_range = (int(hunk_dict['b1']), int(hunk_dict['b2'] or 1), )
