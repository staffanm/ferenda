...
the cookie-less snippet legacy lagen.nu has always used). An API call or an MCP
...
Four things this is built around:
  ...
  * **Its own Matomo site.** `config.MATOMO_SITE_API` is a *different* site id
    ...
    duration worth reading, and mixing them into the human numbers would poison
    both -- legacy lagen.nu, the rebuilt site and this machine traffic are three
    sites in one Matomo, which is also what makes "legacy vs. new" a one-click
    comparison in the All Websites dashboard.
